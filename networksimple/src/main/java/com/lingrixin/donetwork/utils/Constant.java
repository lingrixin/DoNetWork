package com.lingrixin.donetwork.utils;

/**
 * Created by songy on 2017/4/9.
 */

public class Constant {
    public static final String GET_URL = "http://apis.juhe.cn/mobile/get";
    public static final String GET_ALL_URL = GET_URL + "?phone=13552889718&key=de3fa871faf4dd5fbe62be8e37dabb2f";

    public static final String HOST = "http://182.92.118.231/index.php/Api/";
    public static final String LOGIN = HOST + "Users/Login";


    public static final String BAIDU_HTTP = "http://api.fanyi.baidu.com/api/trans/vip/translate";
    public static final String BAIDU_HTTPS = "http://api.fanyi.baidu.com/api/trans/vip/translate";

    public static final String BASE_BAIDU = "http://api.fanyi.baidu.com/";

    public static final String BAIDU_HTTP_ALL = BAIDU_HTTP + "?" + TempUtil.parser(TempUtil.getBaiduMap());


}
